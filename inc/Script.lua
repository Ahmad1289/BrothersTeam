...
if MsgText[1] == "المنشئيين" then 
if not msg.HASSNBOT then return "♦️*┇*هذا الامر يخص {المنشى الاساسي} فقط  \n💥" end
...
if not msg.Admin then return "♦️*┇*هذا الامر يخص {الادمن,المدير,المنشئ,المطور} فقط  \n💥" end
if not MsgText[2] and msg.reply_id then
if redis:get(max..'lock:babak:'..msg.chat_id_) == 'off' then
...
if redis:get(max..'lock:babak:'..msg.chat_id_) == 'on' then
...
if not msg.HASSNBOT then return "♦️*┇*هذا الامر يخص {المطور,المنشى الاساسي} فقط  \n💥" end
if not MsgText[2] and msg.reply_id then
...
if not msg.Creator then return "♦️┇هذا الامر يخص *❴ المنشئ,المطور ❵* فقط  \n💥" end
redis:setex(max..'rulse:wittingg'..msg.sender_user_id_,300,true)
return '📭┇حسننا عزيزي  📤\n🗯┇الان ارسل رابط  للمجموعه 🍃'
end

if MsgText[1] == "الرابط" then
if not redis:get(max..'rulse:msgg'..msg.chat_id_) then 
return "📡*┇* اوه 🙀 لا يوجد رابط ☹️\n🔖*┇*لانشاء رابط ارسل { ضع رابط او انشاء رابط } \n📡" 
else 
return "*🔖┇رابط المجموعة 💯 :*\n"..redis:get(max..'rulse:msgg'..msg.chat_id_) 
end 
end
...
if not msg.Director then return "♦️┇هذا الامر يخص *❴  المطور,المنشئ الاساسي, المنشئ ,المدير  ❵* فقط  \n💥" end
if not redis:get(max..'rulse:msgg'..msg.chat_id_) then 
return "♦️┇عذرا لا يوجد رابط ليتم مسحه \n!" 
end
redis:del(max..'rulse:msgg'..msg.chat_id_)
return "🙋🏻‍♂*┇*أهلا عزيزي "..msg.TheRankCmd.."   \n♦️┇تم حذف الرابط المضاف بنجاح ✓"
end
...
if not msg.SudoBase then return"♦️*┇*هذا الامر يخص {المطور الاساسي} فقط  \n💥" end
return '📑┊الاحصائيات 📮 \n\n📤*┊*عدد الكروبات المفعله : '..redis:scard(max..'group:ids')..'\n📈*┊*عدد المشتركين في البوت : '..redis:scard(max..'users')..'\n📡'
end
---------------[End Function data] -----------------------
...
💭┇اضف منشئ اساسي ☜ تنزيل منشئ اساسي
💭┇اضف منشئ ☜ تنزيل منشئ 
💭┇اضف المدير ☜ تنزيل المدير
💭┇اضف ادمن ☜ تنزيل ادمن 
💭┇اضف مميز ☜ تنزيل مميز 
ـــــــــــــــــــــــــــــــــــــــــــــــــــــــــ
...
💭┇الحمايه ‿ لعرض كل الاعدادات  
💭┇الوسائط ‿ لعرض اعدادات الميديا 
💭┇القوانين ‿ لعرض  القوانين 
...
💭┇ايدي العضو بالرد ⇜ لمعرفه معلومات حسابه 
💭┇ايدي العضو بالمعرف ⇜ لمعرفه معلومات حسابه 
...
redis:set(max..''..msg.chat_id_,true)  
...
if Text == 'مسح تعيين الايدي' or Text == 'حذف تغير الايدي' then
redis:del("KLISH:ID")
sendMsg(msg.chat_id_,msg.id_,"🙋🏼‍♂️*¦* أهلا عزيزي "..msg.TheRankCmd.."\n📡*¦* تم حذف كليشه الايدي \n✓")
return false  
end
...
if redis:get(max..'rulse:wittingg'..msg.sender_user_id_) then --- استقبال القوانين
redis:del(max..'rulse:wittingg'..msg.sender_user_id_) 
redis:set(max..'rulse:msgg'..msg.chat_id_,Flter_Markdown(msg.text)) 
...
elseif msg.edited and redis:get(max..'lock_edit'..msg.chat_id_) then -- قفل الميديا
Del_msg(msg.chat_id_,msg.id_,function(arg,data)
...
elseif Text== "فتح الفارسيه بالتقييد"  then return sendMsg(msg.chat_id_,msg.id_,"📡┇تم فتح الفارسيه ")
elseif Text== "قفل الفارسيه بالتقييد"  then return sendMsg(msg.chat_id_,msg.id_,"📡┇الفارسيه بالتأكيد تم قفله")
...
elseif Text== "شسمج"  then return sendMsg(msg.chat_id_,msg.id_,"عوف اسمي وانطيني رقمك بلخاص هههه🙊")
...
elseif Text== "قفل الفارسيه"  then return sendMsg(msg.chat_id_,msg.id_,"⚙┇ تم قفل الفارسيه ✓\n✓️")
elseif Text== "فتح الفارسيه"  then return sendMsg(msg.chat_id_,msg.id_,"⚙┇ تم فتح الفارسيه ✓\n✓️")
