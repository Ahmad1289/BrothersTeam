...
message = message..'*┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ \n\n📋¦ قائمه المنشئ الاساسييين :*\n\n'
local list = redis:smembers(max..'Hussain:'..msg.chat_id_)
if #list == 0 then  
...
GetUserID(v.user_id_,function(arg,data)
redis:hset(max..'username:'..data.id_, 'username', ResolveUser(data))
redis:sadd(max..':MONSHA_BOT:'..msg.chat_id_,data.id_)
end)
...
elseif redis:sismember(max..':Hussain:'..ChatID,UserID) then 
...
if redis:sismember(max..':Hussain:'..ChatID,UserID) then 
return SendMention(ChatID,UserID,MsgID,'🙍🏻‍♂┇العضو » ❪ '..USERNAME..' ❫\n🎫┇الايدي » ❪ '..UserID..' ❫\n💯┇انه بالتأكيد منشئ  \n✓️',17,USERCAR) 
end
redis:hset(max..'username:'..UserID,'username',Resolv)
redis:sadd(max..':Hussain:'..ChatID,UserID)
return SendMention(ChatID,UserID,MsgID,'🙍🏻‍♂┇ العضو » ❪ '..USERNAME..' ❫\n🎫┇الايدي » ❪ '..UserID..' ❫\n💯┇تمت ترقيته ليصبح منشئ في البوت \n✓️',17,USERCAR) 
end

if cmd == "hassann" then
if not redis:sismember(max..':Hussain:'..ChatID,UserID) then
return SendMention(ChatID,UserID,MsgID,'🙍🏻‍♂┇العضو » ❪ '..USERNAME..' ❫\n🎫┇الايدي » ❪ '..UserID..' ❫\n💯┇انه بالتأكيد ليس منشئ  \n✓️',17,USERCAR) 
end
redis:srem(max..':Hussain:'..ChatID,UserID)
return SendMention(ChatID,UserID,MsgID,'🙍🏻‍♂┇العضو » ❪ '..USERNAME..' ❫\n🎫┇الايدي » ❪ '..UserID..' ❫\n💯┇تم تنزيله من المنشئ  \n✓️',17,USERCAR) 
end
...
if redis:sismember(max..':MONSHA_BOT:'..ChatID,UserID) then 
return sendMsg(ChatID,MsgID,'🙍🏻‍♂*┇*العضو » ❪ '..UserName..' ❫\n🎟*┇*لايدي » ❪ `'..UserID..'` ❫\n💯*┇*انه بالتأكيد منشئ الاساسي في البوت \n✓️') 
end
...
.."\n📝┇الميديا ⇜ "..(redis:get(max..'lock_edit'..msg.chat_id_) or 'false')
